...
export const MultimodalPreview: React.FC<MultimodalPreviewProps> = ({
  ...
  if (
    ...
    return (
      <div className={cn("relative inline-block", className)}>
        <Image
          src={url}
          alt={String(block.metadata?.name || "imagem enviada")}
          className={imgClass}
  ...
  if (block.type === "file" && block.mimeType === "application/pdf") {
    const filename =
      block.metadata?.filename || block.metadata?.name || "Arquivo PDF";
    return (
      <div
        className={cn(
  ...
  return (
    ...
    >
      ...
      <span className="truncate text-xs">Tipo de arquivo não suportado</span>
      {removable && (
        <button
          type="button"
