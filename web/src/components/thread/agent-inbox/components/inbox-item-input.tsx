...
function EditActionCard({
  ...
  const header = editResponse.acceptAllowed ? "Editar/Aprovar" : "Editar";
  const buttonText =
...
function RejectActionCard({
  ...
  return (
    <div className="flex w-full max-w-full flex-col items-start gap-4 rounded-xl border border-gray-300 p-6">
      <div className="flex w-full items-center justify-between">
        <p className="text-base font-semibold text-black">Rejeitar</p>
      ...
      <div className="flex w-full flex-col items-start gap-[6px]">
        <p className="min-w-fit text-sm font-medium">Motivo</p>
        <Textarea
          disabled={isLoading}
          ...
          onKeyDown={handleKeyDown}
          rows={4}
          placeholder="Compartilhe feedback com o agente..."
        />
      </div>
...
export function InboxItemInput({
  ...
  const onEditChange = (
    ...
    if (
      ...
      toast.error("Erro", {
        description: "Não foi possível atualizar os valores editados.",
        richColors: true,
        closeButton: true,
      });
      return;
  ...
  return (
    <div className="flex w-full max-w-full flex-col items-start justify-start gap-2">
      ...
      <div className="flex w-full flex-col items-stretch gap-2">
        ...
        {isLoading && (
          <p className="text-sm text-gray-600">Enviando decisão...</p>
        )}
        {selectedSubmitType && supportsMultipleMethods && (
          <p className="text-xs text-gray-500">
            Selecionado atualmente: {prettifyText(selectedSubmitType)}
          </p>
        )}
      </div>
    </div>
